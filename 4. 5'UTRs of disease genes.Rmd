...
mst_ddG2P<-read.csv(file="/Users/nwieder/OneDrive - Nexus365/NWIEDER DPhil Projects/1_5'UTR_summary_stats/MANE Analysis/5' UTR analysis/3.  5 UTR in disease genes/gene subsets databases/D2P_18.02.21/DDG2P_18_2_2021.csv.gz")
ddG2P <- mst_ddG2P
#bring in MANE summary file and merge to get transcript/gene IDs
mane_summary <- read.delim(file = "/Users/nwieder/OneDrive - Nexus365/NWIEDER DPhil Projects/1_5'UTR_summary_stats/MANE Analysis/5' UTR analysis/5UTr paper/code for paper/MANE v1/MANE.GRCh38.v1.0.summary.txt.gz")
mane <- mane_summary
...
copy_census <-  read.csv("/Users/nwieder/OneDrive - Nexus365/NWIEDER DPhil Projects/1_5'UTR_summary_stats/MANE Analysis/5' UTR analysis/3.  5 UTR in disease genes/gene subsets databases/cancer gene lists/Census_allMon Feb 22 14_01_37 2021.csv")
...
ds <- read_excel("/Users/nwieder/OneDrive - Nexus365/NWIEDER DPhil Projects/1_5'UTR_summary_stats/MANE Analysis/5' UTR analysis/5UTr paper/code for paper/MANE v1/collins DS.xlsx")#18641
...
####note for plotting this is all saved so dont need to run "/Users/nwieder/OneDrive - Nexus365/NWIEDER DPhil Projects/1_5'UTR_summary_stats/MANE Analysis/5' UTR analysis/5UTr paper/code for paper/MANE v1/disease_lengths_m1.txt"
...
#ggplot(length_plot, aes(x=UTR_length, y=group, fill=disease)) + geom_boxplot(alpha=0.8) + theme_light() + xlab("5'UTR Length (bp)") + ylab("Disease Gene Group") + theme(legend.position="none") +xlim(0,1000) + facet_grid(disease ~ ., scales = "free", space = "free", labeller = labeller(disease=label_wrap_gen(width=20))) + scale_fill_manual(values=c("#5D5B5D","#F6C2FF","#E75CFF", "#8B00A3")) + theme(axis.text.y = element_text(size = 10)) + theme(strip.background =element_rect(fill="gray47"))+ theme(strip.text = element_text(colour = 'white', size = '6', face="bold")) + geom_vline(xintercept = 136, colour="black",linetype = "dotted")

#all teh dark purple 5utr colour
ggplot(length_plot, aes(x=UTR_length, y=group, fill=disease)) + geom_boxplot(alpha=0.8) + theme_light() + xlab("5'UTR Length (bp)") + ylab("Disease Gene Group") + theme(legend.position="none")  + facet_grid(disease ~ ., scales = "free", space = "free", labeller = labeller(disease=label_wrap_gen(width=20))) + scale_fill_manual(values=c("#5D5B5D","#793A92","#793A92", "#793A92")) + theme(axis.text.y = element_text(size = 10)) + theme(strip.background =element_rect(fill="gray47"))+ theme(strip.text = element_text(colour = 'white', size = '6', face="bold")) + geom_vline(xintercept = 136, colour="black",linetype = "dotted") + theme(text = element_text(size = 20)) + coord_cartesian(xlim=c(0, 1000))

#try raincloud for this
#this works but i cant limit x axis
source("https://gist.githubusercontent.com/benmarwick/2a1bb0133ff568cbe28d/raw/fb53bd97121f7f9ce947837ef1a4c65a73bffb3f/geom_flat_violin.R") 

ggplot(data = length_plot, aes(y=UTR_length, x = group)) +   geom_flat_violin(position = position_nudge(x = 0.2, y = 0), alpha = 0.8, fill="#793A92") +     geom_point(aes(y = UTR_length, fill = group), position = position_jitter(width = 0.15), size = 1, alpha = 0.1, color="grey48") +     geom_boxplot(width = 0.2, outlier.shape = NA, alpha = 0.5, fill="#793A92") +     labs(x = "5'UTR Length (bp)") + guides(fill = FALSE, color = FALSE) +theme_light()  + theme(text = element_text(size = 15)) + coord_flip() + geom_hline(yintercept = 136, colour="black",linetype = "dotted") + facet_grid(disease ~ ., scales = "free", space = "free", labeller = labeller(disease=label_wrap_gen(width=20))) + facet_grid(disease ~ ., scales = "free", space = "free", labeller = labeller(disease=label_wrap_gen(width=20))) + theme(axis.text.y = element_text(size = 10)) + theme(strip.background =element_rect(fill="gray47"))+ theme(strip.text = element_text(colour = 'white', size = '6', face="bold"))  + ylab("5'UTR Length (bp)") + xlab("Disease Gene Group") 
#need to do coordflip with ylim in it i think
...
#npt sure about sizing of text seems diff to the ones i have? try just leaving it out and use the native

#try do it with x and y flipped with out using coord flip
ggplot(data = length_plot, aes(x=UTR_length, y = group)) +   geom_flat_violin(position = position_nudge(x = 0.2, y = 0), alpha = 0.8, fill="#793A92") +     geom_point(aes(x = UTR_length, fill = group), position = position_jitter(width = 0.15), size = 1, alpha = 0.1, color="grey48") +     geom_boxplot(width = 0.2, outlier.shape = NA, alpha = 0.5, fill="#793A92")  + guides(fill = FALSE, color = FALSE) +theme_light()  + theme(text = element_text(size = 15)) + geom_hline(yintercept = 136, colour="black",linetype = "dotted") + facet_grid(disease ~ ., scales = "free", space = "free", labeller = labeller(disease=label_wrap_gen(width=20))) + facet_grid(disease ~ ., scales = "free", space = "free", labeller = labeller(disease=label_wrap_gen(width=20))) + theme(axis.text.y = element_text(size = 10)) + theme(strip.background =element_rect(fill="gray47"))+ theme(strip.text = element_text(colour = 'white', size = '6', face="bold"))  + xlab("5'UTR Length (bp)") + ylab("Disease Gene Group")
#now not showing violin bit

    



#goes crazy when i add coord to limit x axis


#not showing the violin bit..



#save
#write.table(length_plot, "/Users/nwieder/OneDrive - Nexus365/NWIEDER DPhil Projects/1_5'UTR_summary_stats/MANE Analysis/5' UTR analysis/5UTr paper/code for paper/MANE v1/disease_lengths_m1.txt")
...
#write.table(orf_plot, "/Users/nwieder/OneDrive - Nexus365/NWIEDER DPhil Projects/1_5'UTR_summary_stats/MANE Analysis/5' UTR analysis/5UTr paper/code for paper/MANE v1/disease_orfs4plot_m1.txt")
...
#this is green
#ggplot(phylop_plot, aes(x=agg_PGS, y=group, fill=disease)) + geom_boxplot(alpha=0.8) + theme_light() + ylab("Gene Group") + xlab("Mean PhyloP per 5'UTR")  +theme(legend.position="none") + facet_grid(disease ~ ., scales = "free", space = "free", labeller = labeller(disease=label_wrap_gen(width=20))) + scale_fill_manual(values=c("#5D5B5D","#C2FFC4","#00E007", "#005603")) + theme(axis.text.y = element_text(size = 10))+ theme(strip.background =element_rect(fill="gray47"))+ theme(strip.text = element_text(colour = 'white', size = '6', face="bold"))+ theme(strip.text = element_text(colour = 'white', size = '7', face="bold")) + geom_vline(xintercept = 0.2765763, linetype = "dotted")

#all dark purple colour
ggplot(phylop_plot, aes(x=agg_PGS, y=group, fill=disease)) + geom_boxplot(alpha=0.8) + theme_light() + ylab("Gene Group") + xlab("Mean PhyloP per 5'UTR")  +theme(legend.position="none") + facet_grid(disease ~ ., scales = "free", space = "free", labeller = labeller(disease=label_wrap_gen(width=20))) + scale_fill_manual(values=c("#5D5B5D","#793A92","#793A92", "#793A92")) + theme(axis.text.y = element_text(size = 10))+ theme(strip.background =element_rect(fill="gray47"))+ theme(strip.text = element_text(colour = 'white', size = '6', face="bold"))+ theme(strip.text = element_text(colour = 'white', size = '7', face="bold")) + geom_vline(xintercept = 0.2765763, linetype = "dotted")
...
#p3.131061e-65
#think how to make this a function then can run on all disease groups in one go!!
...
#go back and add subgroup...

##up to here
#remember parse from tidyr if i want
...
#6. Recessive Genes vs Middle LEOUF genes -done 
```{r}
