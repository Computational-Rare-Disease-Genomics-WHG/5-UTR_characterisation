---
title: "Untitled"
author: "Nechama Wieder" unless stated otherwise
date: "2022-11-22"
output: html_document
---

Using MANE v1, here we gather various other 5'UTR features and information

Load Libraries

1. PhyloP scores
  1.1 PhyloP scores for 5UTR
  1.2 uORF starts PhyloP
  1.3 uORF stops PhyloP
  1.4 Start-stops PhyloP
2. CADD scores 
3. RNAfold   
4. CAGE peak  
5. Ribo-seq uORFs 
...
threes<-subset(df1, type=="three_prime_UTR")
mean_fives<-(((sum(fives$end)) - (sum(fives$start)))/(nrow(fives)))
mean_threes<-(((sum(threes$end)) - (sum(threes$start)))/(nrow(threes)))
...
#need a df of pred uORFs with positions
###change to correct input when finish mane file
...
#the uorf starts may be split by intron so bit of fiddling to get the exact bits
...
#need to think of way to identify which is the one we want.. help. needs to be TAA/TAG/TGA i guess thats a strt
...
# need to get the smORF uORF codons using their locations - remember they are split by introns.
...
```
#6. Translational Efficiency (Noderer Et al 2014) - done
```{r message=FALSE, error=FALSE, warning=FALSE}
